...
async function hashLeftRight(left, right) {
  ...
  const { hash } = await bb.poseidon2Hash({ inputs: [hexToBuffer(left), hexToBuffer(right)] });
...
export class PoseidonTree {
    ...
    if (defaultLeaves.length > 0) {
      ...
      for (let level = 1; level <= this.levels; level++) {
        ...
        for (let i = 0; i < numNodes; i++) {
          const left =
            this.storage.get(PoseidonTree.indexToKey(level - 1, 2 * i)) || this.zeros[level - 1];
          const right =
            this.storage.get(PoseidonTree.indexToKey(level - 1, 2 * i + 1)) || this.zeros[level - 1];
  ...
  proof(index) {
    ...
    this.traverse(index, (level, currentIndex, siblingIndex) => {
      const sibling =
        this.storage.get(PoseidonTree.indexToKey(level, siblingIndex)) || this.zeros[level];
  ...
  async update(index, newLeaf, isInsert = false) {
    ...
    await this.traverseAsync(index, async (level, currentIndex, siblingIndex) => {
      const sibling =
        this.storage.get(PoseidonTree.indexToKey(level, siblingIndex)) || this.zeros[level];
      const [left, right] =
        currentIndex % 2 === 0 ? [currentElement, sibling] : [sibling, currentElement];
...
// Same ZERO_VALUES as IncrementalMerkleTree.sol zeros()
const ZERO_VALUES = [
